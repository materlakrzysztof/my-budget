...
test("deleting an expense via the confirmation dialog updates the list and the summary total", async ({ page }) => {
  ...
  await expenseDialog(page, "add").getByLabel("Kategoria").selectOption({ label: "Media" });
  ...
  await expect(expenseRowFor(page, "Media").filter({ hasText: "30,00 USD" })).toBeVisible();
  ...
  await expenseDialog(page, "add").getByRole("button", { name: "Dodaj wydatek" }).click();
  await expect(expenseRowFor(page, "Media").filter({ hasText: "45,00 USD" })).toBeVisible();
  await page.goto("/dashboard");
  await expect(page).toHaveURL(/\/dashboard$/);
  await expect(summaryRowFor(page, "Media")).toContainText("75,00 USD");

  await page.goto("/expenses");
  await expect(page).toHaveURL(/\/expenses$/);

  const targetRow = expenseRowFor(page, "Media").filter({ hasText: "45,00 USD" });
  ...
  await expect(expenseRowFor(page, "Media").filter({ hasText: "45,00 USD" })).toHaveCount(0);
  await expect(expenseRowFor(page, "Media").filter({ hasText: "30,00 USD" })).toBeVisible();
  await page.goto("/dashboard");
  await expect(page).toHaveURL(/\/dashboard$/);
  await expect(summaryRowFor(page, "Media")).toContainText("30,00 USD");
  await expect(summaryRowFor(page, "Media")).not.toContainText("75,00 USD");
});
