...
A personal household budget tracker: log expenses, organize them into categories, and see a month-end summary ranking which categories drove the most spending. Scaffolded from [10x Astro Starter](https://github.com/przeprogramowani/10x-astro-starter); product scope lives in `context/foundation/prd.md`.
...
Beyond Supabase Auth's built-in `auth.users` table, MyBudget adds its own tables (categories, expenses) via `supabase/migrations/`. Run `npx supabase db reset` after pulling new migrations to apply them locally.
